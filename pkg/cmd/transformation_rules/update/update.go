...
func NewCmdTransformationRulesUpdate(client *cip.APIClient) *cobra.Command {
	var (
	...
	cmd := &cobra.Command{
		Use:   "update",
		Short: "Update an existing transformation rule.",
		Long:  "All properties specified in the request are replaced. Missing properties will remain the same.",
		Run: func(cmd *cobra.Command, args []string) {
			update(client, addOrReplaceTransformationType, aggregateOn, aggregateOnTransformationType, dimensionToReplace,
				enabled, id, name, retention, selector, transformedMetricsRetention, value)
		},
	}
	...
	cmd.Flags().StringVar(&id, "id", "", "Specify the identifier of the transformation rule to update.")
...
func update(client *cip.APIClient, addOrReplaceTransformationType bool, aggregateOn []string, aggregateOnTransformationType bool,
	dimensionToReplace string, enabled bool, id string, name string, retention int64, selector string,
	transformedMetricsRetention int64, value string) {
	...
	data, response, err := client.UpdateTransformationRule(body, id)
	if err != nil {
