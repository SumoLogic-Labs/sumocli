...
func NewCmdUnlockUser(client *cip.APIClient, log *zerolog.Logger) *cobra.Command {
	var id string
	cmd := &cobra.Command{
		Use:   "unlock",
		Short: "Unlocks a Sumo Logic user account",
		Run: func(cmd *cobra.Command, args []string) {
			unlockUser(id, client, log)
		},
	}
...
func unlockUser(id string, client *cip.APIClient, log *zerolog.Logger) {
	httpResponse, errorResponse := client.UnlockUser(id)
	if errorResponse != nil {
		log.Error().Err(errorResponse).Msg("failed to unlock user")
	} else {
